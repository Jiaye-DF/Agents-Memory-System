...
2. **依序讀取 `docs/Design-Base/` 底下所有規範檔**（順序如下），作為掃描的權威規則來源：
   ...
   - `40-permission.md`
   - `90-code-fixed.md`
3. 讀取 `docs/Tasks/v*/tasks-v*.md`（依版號分目錄），了解當前版本預期的交付範圍與 Definition of Done。
4. 讀取當前版本 `docs/Tasks/v{版本}/fixed.md`，作為「已修過的 bug 清單」，掃描時用於排除已處理項目，避免在報告中重複列為違規。
5. 所有輸出一律使用**繁體中文**。
...
- **當前 Task 進度**：對照 `docs/Tasks/v*/tasks-v*.md` 最新版本的 Definition of Done，檢查哪些項目已交付。
...
- **有指定版本**：標題 `# 專案掃描報告（v{版本} 後）`，版本號使用該目錄內最新子版本（例：`v1.2/` 下存在 `tasks-v1.2.4.md` → 標題寫 `v1.2.4 後`）。
...
> 對照 `docs/Design-Base/*`、`docs/Tasks/v{版本}/tasks-v*.md` 與 `docs/Tasks/v{版本}/fixed.md`。
>
...
若該違規已記錄於當前版本 `fixed.md`，於該項目尾端附註 `— 見 fixed.md §N`，並視情況改以 `[x]` 標記，不重複列為待處理。
...
- 規範衝突時優先順序：**`docs/Tasks/v*/tasks-v*.md`** > **`docs/Design-Base/*`** > **`CLAUDE.md`** > 其他。
