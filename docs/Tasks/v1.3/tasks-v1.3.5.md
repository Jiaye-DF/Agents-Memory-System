...
> **狀態：進行中（程式碼完成：commit 99b05a4 + 後續 Phase 8 回填, 2026-04-25；runtime 行為驗收待完成）** — Phase 0 ~ 8 程式碼完成；DB migration 套用、worker 實際消費 queue、跨層生命週期實測需於使用者本機 `/dev-up` 啟動環境後執行。
...
> 本檔案 Phase 0 ~ 7 的程式碼層 checkbox 即為實作交付清單；smoke / curl / 瀏覽器互動類驗證請見 acceptance 檔案對應章節。
