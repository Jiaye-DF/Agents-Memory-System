...
- [ ] V43 套用後 `system_setting` 有 5 個 `classifier.*` keys，COMMENT 齊全
- [ ] 任一 key 被人工修改後（直接改 DB），下一次 chat 請求會讀到新值（無快取殘留）
- [ ] `classifier.thresholds` 為合法 JSON、解析失敗時 fallback 到模組內建預設

### Classifier Service

- [ ] greeting whitelist 命中 → `route="skip"`、`matched_rule="greeting_whitelist:<word>"`
- [ ] 純 emoji 訊息（如「👍🎉」）→ `route="skip"`
- [ ] `len(content) < min_length` → `route="skip"`
- [ ] 短文字無歷史輪次（`history_turns=0`、長度 < `cheap_max_length`） → `route="cheap"`
- [ ] 長文字 / 多輪次 → `route="expensive"`
- [ ] `classifier.enabled=false` → 全部訊息 `route="expensive"`、`reason="classifier_disabled"`
- [ ] 含 image 附件 → `route="expensive"`、`matched_rule="image_attachment"`，且不論 `classifier.enabled` 為何皆生效
- [ ] 純圖片無文字（`content=""` + image）→ `route="expensive"` 而非 `skip`

### Chat 流程整合

- [ ] skip 路線：使用者收到 `classifier.skip_response_template` 字串、SSE 正常結束、`chat_message` 表有對應 assistant 訊息
- [ ] cheap 路線：實際打到 `classifier.cheap_model`（檢查 `llm_call_log.model` 欄位），其餘 RAG / system prompt 與 expensive 一致
- [ ] expensive 路線：行為與 v1.3.4 上線前完全一致（含 multimodal、graceful degradation）
- [ ] classifier 失敗（exception）時 fallback 到 expensive，使用者體驗不中斷

### Metrics（與 v1.3.0 整合）

- [ ] skip 路線寫入 `llm_call_log`：`route='skip'`、`actual_cost_usd=0`、`baseline_cost_usd > 0`、`model IS NULL`
- [ ] cheap 路線寫入 `llm_call_log`：`route='cheap'`、`model=<cheap_model>`、`actual < baseline`
- [ ] expensive 路線寫入 `llm_call_log`：`route='expensive'`、`model=<agent.model>`、`actual = baseline`（或極接近，視 EXPENSIVE_MODEL_ID 是否與 agent.model 完全一致）
- [ ] 連續 100 則 skip 訊息後查詢 `SELECT SUM(baseline_cost_usd) FROM llm_call_log WHERE route='skip'` 回非零值 — 證明「省的錢」沒有少算
- [ ] `GET /api/v1/admin/metrics/cost?group_by=route` 回三組 breakdown（skip / cheap / expensive），各組 `calls` / `actual` / `baseline` 數值合理

### Multimodal

- [ ] 圖片 + 文字訊息走 expensive，使用者實際收到 vision model 對圖片的描述
- [ ] 圖片 + greeting 文字（"hi" + image）→ 走 expensive（**不**被 greeting 規則攔截）
- [ ] 圖片 + 不支援 vision 的 model：走既有 v1.1.6 graceful degradation、`route='expensive'` 仍寫入 log

### API 文件

- [ ] Swagger `/api/docs` 沒有新端點需要露出（本版未開新 API），但確認既有 `/api/v1/admin/metrics/cost`（v1.3.0）的 `route` query value 文件包含 `skip` / `cheap` / `expensive`

### 規範對齊

- [ ] 遵守 [CLAUDE.md](../../../CLAUDE.md)：時間 UTC+8、繁中註解、API 文件路徑 `/api/docs`
- [ ] 程式碼 / commit / log 訊息一律用「路由分類器 / classifier」措辭，未出現「意圖推斷器 / Intent Classifier」
- [ ] [docs/Arch/00-memory-system.md §4](../../Arch/00-memory-system.md) 命名澄清表的決策被本版實作對齊（output 是動作不是標籤）

### Follow-up 待確認

- [ ] 誤判率指標需求記錄於 fixed.md（待 v1.3.0 metrics 累積 7+ 天資料後再開 task）
- [ ] cheap 路線命中率若 < 5%（規則太保守）→ 開 task 評估升級到 local DistilBERT
- [ ] 觀察一週後檢視 `route` 分佈與 `saved_pct`，若 `saved_pct < 20%` 重新檢討規則閾值
