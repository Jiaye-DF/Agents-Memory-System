...
- [ ] `migrations/sql/V39__create_session_agent.sql`
...
- [ ] V39 同檔末段 `INSERT INTO session_agent (chat_session_uid, agent_uid, role) SELECT chat_session_uid, agent_uid, 'primary' FROM chat_session WHERE agent_uid IS NOT NULL ON CONFLICT DO NOTHING`
- [ ] 加註解說明此 INSERT 為一次性遷移，後續寫入由 application 層處理

### 0-3 V40：`chat_message.responding_agent_uid`

- [ ] `migrations/sql/V40__add_chat_message_responding_agent.sql`
...
- [ ] 同檔回填既有資料：`UPDATE chat_message SET responding_agent_uid = cs.agent_uid FROM chat_session cs WHERE chat_message.chat_session_uid = cs.chat_session_uid AND chat_message.role = 'assistant' AND chat_message.responding_agent_uid IS NULL AND cs.agent_uid IS NOT NULL`

### 0-4 V41：`chat_session.agent_uid` 改 nullable + 標 deprecated

- [ ] `migrations/sql/V41__alter_chat_session_agent_uid_deprecated.sql`
...
- [ ] `migrations/sql/V42__seed_agent_templates_v13.sql`
...
- [ ] 確認 Flyway 套用順序：V37 → V38（v1.3.0）→ V39 → V40 → V41 → V42
- [ ] 本機 `docker compose up flyway` 套用無 out-of-order 錯誤（驗收於 Phase 7）
...
- [ ] `backend/app/models/session_agent.py`：`SessionAgent(Base)`
...
- [ ] `backend/app/models/chat_message.py` 加 `responding_agent_uid: Mapped[uuid.UUID | None] = mapped_column(Uuid, nullable=True)`
- [ ] 註解：`assistant 訊息綁一個 Agent；user 訊息為 NULL`

### 1-4 Schema（`backend/app/schemas/chat/schemas.py` 或對應位置）

- [ ] `SessionAgentItem`：`{ session_agent_uid, agent_uid, agent_name, agent_avatar_url?, role: Literal['primary','member'], created_at }`
- [ ] `ChatSessionDetailResponse` 加 `agents: list[SessionAgentItem]`
...
- [ ] `ChatMessageItem` / `ChatMessageResponse` 加 `responding_agent_uid: Optional[UUID]`、`responding_agent: Optional[AgentBrief]`（`AgentBrief` 至少含 `agent_uid` / `name` / `avatar_url`）
- [ ] `ChatMessagePostRequest` 加 `mentioned_agent_uid: Optional[UUID]`（前端 @mention 解析後傳入）

### 1-5 Agent Template Schema

- [ ] 確認 `AgentTemplateResponse` 既有欄位涵蓋 `template_key='general' | 'long-analysis' | 'summary'`，無需 schema 改動
- [ ] 若前端依賴特定 enum，於 schema 補上 `template_key: str`（不收斂為 Literal，避免 admin 新增 template 後型別出包）
...
- [ ] `add(session_uid, agent_uid, role='member')`：UPSERT；若存在但 `is_deleted=True` 則復活
- [ ] `remove(session_uid, agent_uid)`：軟刪；若該筆為 `primary` 且 session 仍有其他 member，需呼叫者額外處理 promote（service 層責任）
- [ ] `list_by_session(session_uid)`：JOIN `agent` 取 `name` / `avatar_url`，過濾 `is_deleted=False`
- [ ] `get_primary(session_uid) -> SessionAgent | None`
- [ ] `set_primary(session_uid, agent_uid)`：將指定 agent_uid 設為 primary，其他改為 member（同 transaction）
- [ ] `is_member(session_uid, agent_uid) -> bool`：權限檢查用

### 2-2 `chat_session_service` 改造

- [ ] `create_session(payload, user)`：
...
- [ ] `get_session_detail(session_uid, user)`：附帶 `session_agent_repository.list_by_session` 結果
- [ ] **新增** `add_agent_to_session(session_uid, agent_uid, user)`：權限驗證 → repository.add；超過 `multi_agent.max_per_session` 時回 422
- [ ] **新增** `remove_agent_from_session(session_uid, agent_uid, user)`：
  - 不允許移除最後一個 Agent（回 422 `cannot_remove_last_agent`）
  - 移除 primary 時，自動 promote 加入時間最早的 member 為 primary
- [ ] **新增** `promote_primary(session_uid, agent_uid, user)`：呼叫 repo `set_primary`

### 2-3 `chat_message_service` 改造（@mention 解析 + 序列分派）

- [ ] `send_message(session_uid, content, attachments, mentioned_agent_uid, user)`：
  - 解析 mention：
...
- [ ] `list_messages(session_uid, ...)`：response 附帶 `responding_agent` 物件（JOIN agent 取 name / avatar_url），無對應時為 None
- [ ] LLM 呼叫紀錄沿用 v1.3.0 `llm_metering.py` wrapper，`agent_uid` 帶入既有 metering 欄位（若 v1.3.0 已有此欄位）

### 2-4 與既有 RAG / Memory 的整合

- [ ] `chat_memory` 寫入路徑：保持 session scope（與 `responding_agent_uid` 無關），不新增 agent 維度欄位（v1.3.5 才考慮 user/project memory）
- [ ] Skill 載入路徑：`mentioned_agent` 的 skill_uids 沿用既有 `_skill_prompt_text` 邏輯，多 Agent 不共享 skills
...
- [ ] `GET /api/v1/chat/sessions/{uid}`：response 加 `agents`
- [ ] **新增** `POST /api/v1/chat/sessions/{uid}/agents`：
  - body：`{ agent_uid: UUID }`
  - response：更新後的 `agents` list
- [ ] **新增** `DELETE /api/v1/chat/sessions/{uid}/agents/{agent_uid}`
- [ ] **新增** `PATCH /api/v1/chat/sessions/{uid}/agents/{agent_uid}/promote`：將 agent 設為 primary

### 3-2 Message API

- [ ] `POST /api/v1/chat/sessions/{uid}/messages`：
  - body 加 `mentioned_agent_uid: Optional[UUID]`
  - response 內 assistant message 含 `responding_agent_uid` / `responding_agent`
- [ ] `GET /api/v1/chat/sessions/{uid}/messages`：每筆 message 含 `responding_agent_uid` / `responding_agent`

### 3-3 Skill 推薦 placeholder（接 v1.3.6 用）

- [ ] **新增**（占位 endpoint，回空陣列）`GET /api/v1/agents/{agent_uid}/skill-suggestions?scope=session&scope_uid=<session_uid>`
...
- [ ] Swagger 顯示此 endpoint 並標 `tags=['skills', 'preview']`

### 3-4 Swagger 與文件路徑

- [ ] 所有新增端點有 `summary` / `description` / `response_model`
- [ ] `/api/docs` 顯示完整（沿用 CLAUDE.md 規範路徑）
...
- [ ] `role_prompt` 撰寫遵循 propose §2-1（記憶 / Skill 相關 prompt 一律繁中）— 雖非記憶 prompt，但作為一致性保留

### 4-2 AgentForm UI hint（承 propose §4-3 殘留）

- [ ] `frontend/src/components/agents/AgentForm.tsx`（或對應檔案）：
  - `max_tokens` 欄位下方加 hint：「1024 ≈ 750 中文字；長分析 / code review 建議 4096+ 或留空」
  - 套用 template 時自動帶入該 template 的 `max_tokens`（既有行為若已有則不動）

### 4-3 Admin 範本管理

- [ ] 確認 `/admin/agent-templates` 頁面（若存在）能正常列出新增的 3 筆 template，無前端硬編 enum 撞牆
...
- [ ] `frontend/src/app/(main)/chat/[sessionUid]/page.tsx`（或對應檔案）：
...
- [ ] 不支援多重 @mention（一則訊息只可選一個 Agent，符合決策 #3 序列順序）
- [ ] 若使用者刪除 mention 文字、`mentioned_agent_uid` 同步清空

### 6-2 ChatComposer 整合

- [ ] `frontend/src/components/chat/ChatComposer.tsx`（或對應名）：
  - 文字輸入框 + 附件按鈕 + 送出按鈕既有，本版插入 MentionSelector
  - 送出時 payload：`{ content, attachments, mentioned_agent_uid }`
- [ ] `mentioned_agent_uid` 為 None 時，後端取 primary（與決策 #5 一致）

### 6-3 提示與引導

- [ ] 多 Agent session 第一次進入時顯示 toast / inline tip：「輸入 @ 即可指定 Agent；不指定時由 primary 回覆」
- [ ] 單 Agent session 不顯示 mention 引導，行為與 v1.1 一致
...
- [ ] V39 套用後 `session_agent` 表結構正確、partial unique index 生效（同 session 兩筆 primary 寫入會被擋）
- [ ] V39 既有資料遷移：所有有 `agent_uid` 的 session 在 `session_agent` 都有對應 primary row
- [ ] V40 套用後 `chat_message.responding_agent_uid` 欄位存在；既有 assistant message 已被回填
- [ ] V41 套用後 `chat_session.agent_uid` 為 nullable，COMMENT 含 deprecated 標記
- [ ] V42 套用後 `agent_template` 多 3 筆（`general` / `long-analysis` / `summary`），各自 `max_tokens` 為 2048 / 8192 / 2048
- [ ] Flyway 套用順序 V37 → V38（v1.3.0）→ V39 → V40 → V41 → V42 無 out-of-order 錯誤

### Backend

- [ ] `POST /chat/sessions` 帶 `agent_uids=[uid_a, uid_b, uid_c]` 建立成功；回傳 `agents` 含 3 筆，第一筆 `role='primary'`
- [ ] `POST /chat/sessions` 沿用舊 `agent_uid: <uid>` 仍可建立（向後相容），等同 `agent_uids=[uid]` 且為 primary
- [ ] `GET /chat/sessions/{uid}` response 含 `agents`
- [ ] `POST /chat/sessions/{uid}/agents` 加入新 Agent 成功；超過上限（system_setting `multi_agent.max_per_session`）回 422
- [ ] `DELETE /chat/sessions/{uid}/agents/{agent_uid}`：刪除 primary 自動 promote；刪除最後一個回 422 `cannot_remove_last_agent`
- [ ] `PATCH /chat/sessions/{uid}/agents/{agent_uid}/promote`：原 primary 改為 member，目標改為 primary，partial unique index 不衝突
- [ ] `POST /chat/sessions/{uid}/messages` 帶 `mentioned_agent_uid`：
...
- [ ] `GET /chat/sessions/{uid}/messages` 每筆 assistant message 含 `responding_agent` 物件（含 name / avatar_url）
- [ ] `GET /agents/{agent_uid}/skill-suggestions` stub 回 `{ items: [], hint: 'pending v1.3.6' }`，未登入 401
- [ ] Swagger `/api/docs` 顯示所有新端點與欄位

### Frontend

- [ ] Session header 顯示多 Agent badge（avatar + name + primary 標記）
- [ ] 「+ 新增 Agent」可從使用者可見 Agents 中選；達 5 個上限時按鈕 disable
- [ ] Agent badge 提供移除 / 設為 primary，操作後 UI 即時更新
- [ ] 訊息列 assistant message 顯示來源 Agent 名稱與 avatar；user message 不顯示
- [ ] 輸入框輸入 `@` 觸發 MentionSelector，列出該 session 已掛 Agent
- [ ] 鍵盤上下鍵 / Enter / Esc 操作正確；確認後文字插入 `@AgentName ` 且 payload 帶 `mentioned_agent_uid`
- [ ] 單 Agent session 不顯示 mention 引導；多 Agent session 首次進入顯示 tip
- [ ] Skill 推薦 placeholder 按鈕點擊後顯示空狀態（「目前沒有可推薦的 Skill」），不打擾使用者
- [ ] AgentForm `max_tokens` 欄位下方顯示 hint「1024 ≈ 750 中文字；長分析 / code review 建議 4096+ 或留空」
- [ ] 套用 `general` / `long-analysis` / `summary` template 時自動帶入對應 `max_tokens`（2048 / 8192 / 2048）

### 既有功能不破壞

- [ ] 舊 single-agent session 進入後正常顯示一個 Agent badge、訊息歸屬正確（responding_agent 為原 agent）
- [ ] 舊 `POST /chat/sessions` body 形態（僅 `agent_uid`）仍可建立 session
- [ ] v1.1 chat memory / RAG / SSE memory_updated（若 v1.3.2 已落地）行為不受多 Agent 影響
- [ ] v1.3.0 metrics（`llm_call_log`）每筆呼叫帶正確 `agent_uid`，多 Agent session 可分 Agent 統計成本

### Design-Base / 規範

- [ ] 所有新增 SQL COMMENT 為繁中
- [ ] 所有新增 / 修改 Python / TypeScript 註解為繁中
- [ ] API 文件路徑維持 `/api/docs`，未引入 `/swagger` / `/docs` / `/openapi` 等別名
- [ ] commit message 使用繁中 + `(AI)` 前綴（依 CLAUDE.md）
