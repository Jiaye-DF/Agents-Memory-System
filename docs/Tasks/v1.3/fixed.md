...
## 10. v1.3.6 完成後二輪掃描的中優先補強（既存自 v1.0~v1.3.6）〔2026-04-27 21:24:10〕
...
## 11. owner 欄位命名統一 + 三 service `_ensure_owner` 整合 `core/access.py`（純 refactor，既存自 v1.0）〔2026-04-27 21:47:51〕
...
## 12. llm_model.vendor 欄位持久化 + 修正 provider 欄位混雜（既存自 a5ec97f）〔2026-04-27 22:21:58〕
...
## 13. Dashboard 多軸排序 chip 從垂直改水平排列〔2026-04-28 10:30:00〕
...
## 14. 跨頁作者 chip 點選後過濾結果為空（含空格 username 被 `\s+` 切壞）〔2026-04-28 10:50:00〕

**問題**

`/dashboard`、`/agents` 的「作者：」chip 點選後, 列表瞬間變成 0 筆, 不顯示任何符合資料。
...
3. `matchByTextAndAuthor` 拿 `it-jiaye` 比對 `it-jiaye he 何佳曄` → 不相等 → 0 筆
...
- 用 username 含空格的帳號（如 `IT-Jiaye He 何佳曄`）建立資源, 在 `/dashboard`、`/agents` 點該作者 chip → 列表應正確過濾
...
## 15. Dashboard 文案統一:「你最常用的」→「最常使用」〔2026-04-28 10:55:00〕
...
## 16. FilterNav 文案精簡:「我的收藏」→「收藏」〔2026-04-28 11:05:00〕
...
## 17. 多頁作者 chip 行為統一（補齊 /skills、/scripts UI）〔2026-04-28 11:09:00〕
...
> §13 與 §16 在同一輪 refactor 完成, 拆兩條是因前者是 bug fix（改 util + 修舊頁）、後者是 UI 一致性回補（新加 chip 區塊到原本沒有的頁面）。

---

## 18. Skill / Script 上傳「檔案名稱」欄位顯示使用者命名（非原始檔名）〔2026-04-28 13:07:08〕
...
## 19. 為 Skill 增加「編輯名稱 / 描述」UI 入口 + 補上 Script 詳情頁〔2026-04-28 13:07:08〕

> 這條偏向「規格回補 + UI 一致性」, 同時跟 §18 配套（兩者都是 v1.3 接 SSO display name 後浮現的 UX 缺漏）, 故記在 fixed.md 而非 tasks。
...
## 20. SSO 使用者 refresh 不感知中央狀態 — Azure AD 停用後本地 7 天內仍可存取〔2026-04-28 15:41:16〕
