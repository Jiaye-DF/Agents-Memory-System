...
>
> 註：本文內容原屬「v1.2 propose」，因 v1.2 重規劃為社群互動 / 腳本管理 / 儀錶板排行（見 [v1.2 propose](../v1.2/propose-v1.2.0.md)），本版序號後移為 v1.3。舊 v1.1 延伸 task 文件內若引用「propose-v1.2.0.md §2-X」，應對應至本檔 §2-X。
...
- Agents / Skills / Scripts 的收藏 / 下載統計與 nav
- Sidebar 腳本管理頁、儀錶板首頁熱度 / 收藏排行 filter
...
推薦 A + B 在 v1.3 前段做（獨立於多 Agent 功能）；C 併到 §2-1。
...
> 承接 v1.1.7 commit `98a4d5a` 的 PoC：Redis `agentic:skill:log` stream 累積的 approve/reject 訊號可用於本版 threshold（`min_memory_count` / `topic_concentration` / confidence 下限）的實測調校。
...
等 v1.2（收藏 / 腳本 / 儀錶板排行）實作完、觀察一段時間再啟動 propose 細化。

### 拆分 task 建議

| Task 檔 | 主題 | 前後相依 |
| --- | --- | --- |
| `tasks-v1.3.1.md` | 記憶 / LLM 可觀察性（§2-4 層 1 + §2-5 A+B + §2-6） | 無依賴，可優先 |
| `tasks-v1.3.2.md` | 記憶 UI 即時性（§2-7） | 依賴 3.1 的 worker log 基礎 |
| `tasks-v1.3.3.md` | 多 Agent 對話（§2-1） | 需要 §2-5 C 的 template 預設；§2-8 建議入口由此整合 |
| `tasks-v1.3.4.md` | 分層 Model Classifier（§2-2） | 依賴 3.3 的 session_agent schema；multimodal 路由依賴 v1.1.6 附件系統 |
| `tasks-v1.3.5.md` | User / Project 記憶（§2-3） | 依賴 3.1 可觀察性、§2-3 生命週期設計 |
| `tasks-v1.3.6.md` | Agentic Skill 工廠正式版（§2-8） | 依賴 3.5 跨層記憶 + v1.1.7 PoC 實測數據 |

v1.3.1 是**最低風險高投資報酬**的起點：做完後，其他 task 若出問題都能靠它 debug。

### 與 v1.1 延伸的關係

v1.3 假設 v1.1.6 附件系統與 v1.1.7 Skill PoC 皆已完成（兩者已 release，commit 分別為 `d3e6976` / `98a4d5a`）。
