...
- [x] 建立 v1 路由入口（`backend/app/api/v1/router.py`，前綴 `/api/v1`）
- [x] 建立健康檢查端點 `GET /api/v1/health`（回傳 DB / Redis 連線狀態）
- [x] `backend/app/api/deps.py`：DB session 依賴注入

### 0-3 前端骨架

- [x] 初始化 Next.js 15 專案（App Router, TypeScript 5, TailwindCSS 4）
- [x] Route Group 結構：`(auth)` 登入相關、`(main)` 認證後頁面
- [x] 建立 Redux Store + RTK Query 骨架（`frontend/src/store/`）
- [x] 建立 API Client 層（`frontend/src/lib/api/`）
...
- [x] 建立 Error Boundary（`error.tsx` / `global-error.tsx`）
...
- [x] 全站圓角設計：所有卡片、按鈕、輸入框、Dialog 統一 `rounded-xl`
- [x] 所有可點選元素加上 `hover:cursor-pointer`（按鈕、連結、卡片、圖示按鈕）
- [x] 間距系統使用 Tailwind spacing scale（`p-4`、`gap-6` 等），禁止任意數值

#### 佈景主題

- [x] `globals.css` 定義 CSS Variables 色彩系統
  - 預設：Light / Dark 兩組
  - 自訂：冷色系（cool）/ 暖色系（warm）/ 粉紫色（purple）
- [x] `data-theme` 屬性切換機制，使用者偏好存入 `localStorage`，重新載入自動套用
- [x] 禁止元件內寫死 hex 色碼，一律引用 CSS Variable

#### Layout 系統

- [x] Header 元件（固定頂部，高度固定，背景色跟隨主題）
  - 左側：Sidebar 切換按鈕（☰）+ SVG 圖示 + 「Agents-Platform」文字，點擊回主頁
  - 右側：`{Username}` + 登出按鈕
  - 響應式：`sm` 以下隱藏「Agents-Platform」文字，僅保留 SVG 圖示
- [x] Sidebar 元件（三態循環：完整展開 → 收合僅圖示 → 完全隱藏 → 循環）
  - `md` 以下預設隱藏，點擊以 overlay 方式展開，可關閉
- [x] 統一頁面版型（`(main)/layout.tsx` 控制）
  - 每頁包含 Page Title 區塊 + Main Content 圓角卡片容器（`rounded-xl` + 背景色 + 陰影）
  - 各頁面禁止自行定義外層佈局，僅填充 Main Content 內部

#### 共用元件

- [x] Dialog 元件（禁用 browser `alert()` / `confirm()` / `prompt()`）
...
- [x] 表格元件（小螢幕支援水平捲動或改為卡片式呈現）

#### 響應式設計（RWD）

- [x] Mobile First 為基礎，依序向上擴展
  - 預設（< 640px）：單欄、Sidebar 隱藏為漢堡選單
  - `sm`（≥ 640px）：單欄、間距微調
  - `md`（≥ 768px）：Sidebar 展開、雙欄佈局
  - `lg`（≥ 1024px）：Main Content 加寬
  - `xl`（≥ 1280px）：最大內容寬度，置中顯示
- [x] 所有互動元素觸控區域至少 44x44px（無障礙標準）

### 0-5 資料庫初始化

- [x] Flyway 初始 Migration：啟用 pgvector 擴充（`CREATE EXTENSION IF NOT EXISTS vector`）
- [x] 建立 `updated_at` 自動更新的 Trigger function（共用）
- [x] SQLAlchemy Base model（含必備欄位：pid, uid, is_active, is_deleted, created_at, updated_at）
...
- [x] 分層實作：`api/v1/auth/` → `services/auth_service.py` → `repositories/user_repository.py`
- [x] Pydantic Schemas：RegisterRequest, LoginRequest, TokenResponse, ResetPasswordRequest（account + username + new_password + confirm_password）
- [x] JWT 工具模組（Access Token 15 分鐘、Refresh Token 7 天）
- [x] 密碼 bcrypt 加密（禁止明文儲存或記錄 log）
- [x] 登入失敗鎖定：5 次（鎖 15 分鐘）→ 10 次（鎖 1 小時）→ 15 次以上（永久鎖定，需 admin 解鎖）
- [x] 錯誤訊息不區分「帳號不存在」與「密碼錯誤」（防帳號列舉）
- [x] Redis：Refresh Token 黑名單
- [x] `api/deps.py`：`get_current_user()` 依賴（從 Bearer Token 解析 user_uid + role）

### 1-3 前端頁面

- [x] 登入頁（`/`，掛載於 `(auth)/page.tsx`）
...
- [x] 註冊頁（`/register`）
  ...
  - 密碼強度即時回饋（紅/綠色提示）
- [x] 重設密碼頁（`/reset-password`）
  - 輸入帳號 + 使用者名稱驗證身分
  - 驗證通過後輸入新密碼 + 確認密碼（同註冊密碼規則）
- [x] 登入後導向 `/dashboard`；未登入訪問受保護頁面導向 `/`
- [x] 所有 auth 頁面響應式適配（Mobile First，小螢幕單欄置中）
- [x] RTK Query：auth API endpoints
...
- [x] `require_role(*roles)` 依賴：驗證 Token 中 role 是否符合端點要求（403 若不符）
- [x] 錯誤訊息不洩漏角色資訊
...
- [x] 分層實作：`api/v1/admin/` → `services/` → `repositories/`
- [x] 所有 admin 端點套用 `require_role("admin")`

### 2-3 前端頁面（admin 專用）

- [x] 使用者管理頁（`/admin/users`）
  - 表格列表 + 搜尋欄 + 分頁元件（cursor-based，每頁筆數可選）
  - 小螢幕表格水平捲動或改為卡片式呈現
  - 可查看使用者資訊、指派角色（下拉選單）、解除登入鎖定（Warning Dialog 確認）
- [x] 403 頁面：非 admin 角色訪問 admin 路由時顯示
- [x] 前端路由守衛：非 admin 角色自動導向 403 頁面
...
- [x] 分層實作：`api/v1/agents/` → `services/agent_service.py` → `repositories/agent_repository.py`
- [x] 資源存取控制：member 僅操作 `owner_uid` 為自己的 Agent；admin 可操作全部
- [x] 即使 admin 也不可修改他人 Agent 的公開/私人/刪除狀態（僅能透過 DB）
- [x] 下載功能：動態組合 Agent 設定 + 關聯 Skills 為 AGENTS.md 格式（無關聯 Skills 時僅匯出 Agent 設定）
...
- [x] Agent 建立/編輯頁（`/agents/new`, `/agents/{uid}/edit`）
  - 表單：名稱、描述、語言偏好、風格、身分、角色設定
  - Skills 選擇器（從已有 Skills 中勾選，Checkbox 或 Tag 式）
  - 欄位驗證 inline 紅色提示，送出按鈕 loading 狀態
- [x] Agent 詳情頁（`/agents/{uid}`）：檢視設定 + 下載按鈕
- [x] 公開/私人切換操作（列表頁中操作，即時更新狀態）
- [x] 軟刪除操作（Warning Dialog 確認後執行）
...
- [x] 分層實作：`api/v1/skills/` → `services/skill_service.py` → `repositories/skill_repository.py`
- [x] 上傳處理：
  - 接收檔案後轉為 .zip 格式儲存至 `data/skills/`（此目錄已加入 `.gitignore`）
  - 支援 .md 與相關檔案上傳（必須附帶描述）
  - **拒絕 .exe 檔案**（驗證副檔名 + MIME type）
  - 檔案大小上限 50MB
- [x] 資源存取控制：同 Agents 規則
- [x] 即使 admin 也不可修改他人 Skill 的公開/私人/刪除狀態
