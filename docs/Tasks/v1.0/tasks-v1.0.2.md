...
- [x] `migrations/sql/V12__create_agent_language_table.sql`
...
- [x] `migrations/sql/V13__extend_llm_model.sql`
- 增加：
...
- [x] `migrations/sql/V14__create_system_setting_table.sql`
- 必備欄位遵循 `21-database.md §必備欄位`
- 額外欄位：
...
- [x] 新增 `backend/app/models/agent_language.py`：`AgentLanguage` class，繼承 `Base`
- [x] 新增 `backend/app/models/system_setting.py`：`SystemSetting` class，繼承 `Base`
- [x] `backend/app/models/llm_model.py` 增加 `is_default`、`max_output_tokens` 欄位
- [x] `backend/app/models/__init__.py` 匯出 `AgentLanguage`、`SystemSetting`

### 1-3 Schema

- [x] `backend/app/schemas/agent_languages/schemas.py`（新目錄）
...
- [x] `backend/app/repositories/agent_language_repository.py`：
...
- [x] `backend/app/repositories/llm_model_repository.py` 擴充：
  - `get_default(db)`：取 `is_default = TRUE AND is_active = TRUE` 的模型
- [x] `backend/app/repositories/system_setting_repository.py`：
...
- [x] `backend/app/services/agent_language_service.py`
...
- [x] `backend/app/services/llm_model_service.py` 擴充：
  - `create/update` 處理 `is_default` 切換（同語言邏輯，全表唯一）
- [x] `backend/app/services/agent_service.py` 調整：
...
- [x] `backend/app/services/system_setting_service.py`
...
- [x] Admin 端點套用 `require_role("admin")`
- [x] `PUT /api/v1/admin/settings/{key}` 會驗證 value 是否能依 `value_type` 解析成功
- [x] Swagger `/api/docs` 顯示完整 Schema

### 1-7 Swagger

- [x] 新端點 Request / Response 皆可於 `/api/docs` 正常顯示
...
- [x] `frontend/src/types/agent-languages.ts`：`AgentLanguage`、`AgentLanguageCreateRequest`、`AgentLanguageUpdateRequest`
- [x] `frontend/src/types/models.ts`：`LlmModel` 加 `is_default`、`max_output_tokens`
- [x] `frontend/src/types/system-settings.ts`：`SystemSetting`、`SystemSettingUpdateRequest`、`PublicSettings = Record<string, string | number | boolean>`
- [x] `frontend/src/types/index.ts` re-export

### 2-2 RTK Query

- [x] 新增 `frontend/src/store/agentLanguagesApi.ts`
  - `useListAgentLanguagesQuery`（member）
  - `useListAdminAgentLanguagesQuery` / `useCreate...` / `useUpdate...` / `useDelete...`
- [x] 新增 `frontend/src/store/systemSettingsApi.ts`
  - `useGetPublicSettingsQuery`（member，回傳 `PublicSettings` 字典）
  - `useListAdminSettingsQuery` / `useUpdateSettingMutation`
- [x] `modelsApi.ts`：如前端查詢 models 的地方都要 provide `is_default` 與 `max_output_tokens`，原 hook 不需變

### 2-3 共用工具 / 常數

- [x] `frontend/src/utils/agentTemplates.ts`
...
- [x] `frontend/src/components/ui/MultiSelect.tsx`（可選，若不複用就內嵌於表單）
  - 輸入搜尋 + chip 呈現已選項目
...
- [x] 身分（input）
- [x] 語言偏好（Select，options 來自 `useListAgentLanguagesQuery`；新增表單預設選 `is_default` 的語言）
- [x] 風格（input）
- [x] 角色設定（textarea）

##### 模型參數

- [x] 模型（Select，options 來自 `useListModelsQuery`；新增表單預設選 `is_default` 的模型）
- [x] 溫度：
  - 滑桿 0.0 – 2.0，step 0.1
...
- [x] 回覆格式（Select：Markdown / 純文字 / JSON）
...
- [x] `套用範本` 按鈕（下拉選擇 4 個範本 + `空白`；僅新增頁顯示）
- [x] `從既有 Agent 複製` 按鈕（僅新增頁顯示）
  - 進入 modal，列出使用者可見的 agents（自己的 + 公開），點擊後呼叫 `useGetAgentQuery` 取詳情並預填
- [x] 網址帶 `?from={uid}` 亦觸發同樣流程

#### System Prompt 預覽區

- [x] 右側（或下方）卡片，標題「📄 實際送出的 System Prompt」
- [x] 依 `composeSystemPrompt` 即時渲染 `<pre>`
- [x] 空欄位顯示為灰色 placeholder 文字，不呈現空段

#### 草稿自動儲存

- [x] 新增頁進入時讀取 localStorage，若有草稿**直接自動載入**到表單
- [x] 草稿已載入時，表單上方顯示提示 banner：「已恢復上次未完成的草稿」+ `清除草稿` 按鈕
- [x] 表單變動 throttle 2 秒寫入 localStorage（單一 key：`agent-draft`，每使用者僅一份）
- [x] 成功送出後 / 使用者按「取消」→ 清除草稿
- [x] 編輯頁不啟用草稿

### 2-6 Admin 頁面（語言管理）

- [x] 新增 `frontend/src/app/(main)/admin/agent-languages/page.tsx`
  - 表格列出：code / name / sort_order / is_default（徽章） / is_active / 操作
  - 新增 / 編輯 Dialog
  - 刪除 Warning Dialog
  - 路由守衛：非 admin 導向 403
- [x] Sidebar admin 區增加「語言管理」入口

### 2-7 Admin 頁面（系統設定）

- [x] 新增 `frontend/src/app/(main)/admin/settings/page.tsx`
  ...
  - 路由守衛：非 admin 導向 403
- [x] Sidebar admin 區增加「系統設定」入口

### 2-8 既有頁面調整

- [x] `/admin/models` 頁新增欄位編輯：`is_default` toggle、`max_output_tokens` 輸入
- [x] Dashboard / Agents 列表詳情頁如有顯示「語言」欄位，改為依 code 查表呈現 `name`
...
- [x] V12 / V13 / V14 Migration 可正常套用到空 DB 與既有 DB
- [x] `GET /api/v1/agent-languages` 回傳啟用語言清單（按 sort_order）
- [x] 新增/更新 `agent_language` 時，若 `is_default = TRUE` 會自動把其他項設為 FALSE
- [x] 嘗試刪除 `is_default` 語言 → 400
- [x] `llm_model` 新 `is_default` / `max_output_tokens` 欄位於 admin 端點可編輯
- [x] 系統預設語言與預設模型可由 `get_default` 正確取出
- [x] `GET /api/v1/settings/public` 只回傳 `is_public = TRUE` 的 key，且值依 `value_type` 解析
- [x] `PUT /api/v1/admin/settings/agent.max_skills` 將值改為 `5` 後，建立 Agent 超過 5 個 skill → 400（訊息顯示當前上限為 5）
- [x] `PUT /api/v1/admin/settings/{key}` 收到無法解析的值 → 422
- [x] `/api/docs` 顯示所有新端點
...
- [x] Temperature 滑桿與 preset 按鈕雙向同步
- [x] Max Token 超過選定模型上限時顯示警告
- [x] System prompt 預覽區即時反映 identity / language / style / role_prompt 的變更
- [x] Skills 多選可搜尋、加入、移除；上限動態反映 `GET /api/v1/settings/public` 的 `agent.max_skills`
- [x] admin 於 `/admin/settings` 調整 `agent.max_skills` 後，使用者重新整理 `/agents/new` 新上限即時生效
- [x] 範本套用後所有欄位正確填入
- [x] `?from={uid}` 或點選「從既有 Agent 複製」後表單預填正確
- [x] 關閉 Tab 再開啟 `/agents/new`，草稿**直接自動載入**並顯示 banner 提示
- [x] banner 「清除草稿」按鈕可清除當前草稿並重置表單
- [x] 成功建立 / 按取消後草稿清除
- [x] 編輯頁不出現草稿 banner
- [x] 非 admin 訪問 `/admin/agent-languages` 或 `/admin/settings` 被導向 403

### 回歸

- [x] 既有 Agent（language 為自由文字如 `繁體中文`）於詳情頁 / 列表仍可正常顯示（找不到對應 code 時 fallback 顯示原值）
- [x] 既有建立流程不因新欄位預設值缺失而報錯
- [x] 所有 Migration / API / UI 遵守 `21-database.md` 時區規範（UTC+8）
