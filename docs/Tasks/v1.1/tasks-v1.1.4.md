...
> **狀態：已完成（2026-04-21）**
>
> 實作時 v1.1.1 已 release，故走 Phase 0-2 fallback 路徑；migration 編號使用 V24（V22/V23 已被記憶系統佔用），另新增 V25 seed。
> 另：規格中未提到但實作需要的 `chat_session.owner_user_uid` 欄位一併於 V24 加入並 backfill。
...
- [ ] **V19**：`create_chat_session_table.sql` 將 `chat_project_uid` 欄位改為 `UUID NULL REFERENCES chat_project(chat_project_uid)` —（未採用，實作時 v1.1.1 已 release，改走 0-2 fallback）
- [ ] 對應 `ChatSession` model 欄位：`chat_project_uid: Mapped[uuid.UUID | None]` —（改於 0-2 處理）
- [ ] Seed / 測試資料同步 —（改於 0-2 處理）

### 0-2 若 v1.1.1 已 release（fallback）

- [x] 新增 **V24**：`alter_chat_session_project_uid_nullable.sql`（原規格 V22，因 V22/V23 已被記憶系統佔用，改用 V24）
  ...
  - **額外**：新增 `owner_user_uid` 欄位（NOT NULL + FK + Index），從 `chat_project.owner_user_uid` backfill；規格未提但實作必需（否則游離 session 無法查擁有者）
...
- [x] `app/models/chat_session.py`：`chat_project_uid` type 改為 `Mapped[uuid.UUID | None]`
- [x] `app/schemas/chat/session_schemas.py`
...
- [x] `chat_session_repository.stmt_orphan_by_owner(user_uid)`：`WHERE chat_project_uid IS NULL AND owner_user_uid = ... AND is_deleted = FALSE`
- [x] `chat_session_repository.count_orphan_by_owner(user_uid)`：計算使用者的游離 session 數，用於容量檢查
- [x] `chat_session_repository.update_project(chat_session_uid, new_project_uid_or_none, db)`

### 1-3 Service

- [x] `chat_service.create_session`：
...
- [x] `chat_service.move_session(user_uid, chat_session_uid, new_project_uid_or_none)`：
  - 僅擁有者可呼叫
  - 移入 project：驗證 target project 擁有者一致 + 未超上限
  - 移出 project（設 None）：檢查游離 session 上限
- [x] `chat_service.list_orphan_sessions(user_uid, cursor, limit, db)`
...
- [x] `POST /api/v1/chat/sessions/{uid}/move`
  - body: `{ "chat_project_uid": string | null }`
  - 回 200 + 更新後的 `ChatSessionResponse`

### 1-5 System Setting

- [x] seed 新增 `chat.max_orphan_sessions_per_user = 10`（max 30，admin 可調）
- [x] 在 `/admin/settings` 編輯器顯示與其他 `chat.*` 一起

### 1-6 Delete Project 行為確認

- [x] 刪除 project 時僅 cascade 屬於該 project 的 sessions；游離 sessions 不受影響（既有 `chat_project_uid IS NULL` filter 天然排除）
...
- [x] 調整 `useCreateChatSessionMutation` 型別：`chat_project_uid?: string | null`
- [x] tag：`ChatSessions` 保留，新增 tag `OrphanChatSessions` 以便只對列表精準 invalidate

### 2-3 側欄導航（`components/layout/ChatSidebar.tsx` 新檔或拆分既有）

- [x] 上方區塊「最近對話」：列 `useListOrphanChatSessionsQuery` 結果（限最近 N 筆，點擊 see all 進 `/sessions`）
- [x] 下方區塊「Projects」：列 `useListChatProjectsQuery` 結果
- [x] 每個 project 可展開顯示其 sessions（既有）
- [x] 頂部「新對話」按鈕 → 導向 `/sessions/new`

### 2-4 路由

- [x] `app/(main)/sessions/new/page.tsx`：選 Agent → 建立游離 session → 導向 `/sessions/{uid}`
- [x] `app/(main)/sessions/page.tsx`：列出所有游離 sessions（cursor 分頁，搜尋 / 排序沿用共用 hook）
- [x] `app/(main)/projects/{uid}/sessions/new/page.tsx`：在 project 內開 session（保留既有流程）
- [x] `/sessions/{uid}` 對話頁面顯示所屬 project badge（有的話可點進 project）

### 2-5 Session 詳情

- [x] 加入「移至 project」操作（dropdown 或按鈕）：
  - 空選項「（無，設為游離）」
  - 列出使用者自己的 projects
...
- [x] `propose-v1.1.0.md` §3-1 改寫為「Session 可選擇屬於 Project 或游離」，並補上雙區導航描述
- [x] `propose-v1.1.0.md` §3-2 容量上限表新增 `chat.max_orphan_sessions_per_user`
- [x] `tasks-v1.1.1.md` 已確認決策 #2「Session 必須隸屬 Project」標為 superseded，指向本檔
- [x] `tasks-v1.1.2.md` RAG 段落補註：游離 session 僅檢索自己的 session memory（與 project session 的 session scope 一致，v1.1 層不擴展）

---

## 驗收

- [x] 建立游離 session 不帶 `chat_project_uid`，成功並出現在側欄「最近對話」
- [x] 在 project 內建的 session 出現在 project 展開列表，不出現在「最近對話」
- [x] `move` 端點：游離 → project、project → 游離、project A → project B 全部可行
- [x] 超過 `chat.max_orphan_sessions_per_user` 時建立 / move-to-null 會被拒（400 + 明確訊息）
- [x] 刪除 project 不影響游離 sessions
- [x] admin 不能 move 他人 session（403）
- [x] 游離 session 對話流程（送訊息、streaming、memory worker 寫入、RAG 檢索）完整可用
