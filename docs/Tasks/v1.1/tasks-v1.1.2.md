...
- [ ] **V22**：`create_chat_memory_table.sql`
  - `pid bigserial PK`
...
- [ ] **V23**：`seed_memory_rag_system_settings.sql`
...
- [ ] `app/models/chat_memory.py`：`ChatMemory`
...
於 `pyproject.toml` 加入：
- [ ] `pgvector>=0.3.0`（SQLAlchemy 整合）
- [ ] `openai>=1.50.0`（僅用 embedding client，非 chat）
- [ ] `redis>=5.0.0`（v1.0 既有可重用）

### 1-4 Schema（`app/schemas/chat/memory_schemas.py`）

- [ ] `ChatMemoryResponse`：完整欄位（不含 embedding，太大）
- [ ] `MemoryExtractResult`（Pydantic，用於 small LLM 的 JSON output）：
  ```python
...
- [ ] `create(data, db)`：flush + refresh
- [ ] `list_by_session(session_uid, db)`
- [ ] `soft_delete_by_session(session_uid, db)`：`DELETE FROM chat_memory WHERE chat_session_uid = ?`（無 is_deleted，直接 hard delete 該 session 的記憶，對應 session 軟刪時呼叫）
- [ ] `search_similar(session_uid, query_embedding, top_k, min_score, db)`：
  ```sql
...
- [ ] `async def should_skip(message: ChatMessage, rules: dict) -> bool`
...
- [ ] `async def truncate_for_extraction(content: str, max_tokens: int) -> str`
  - 使用簡單 token 估算（4 字元 ≈ 1 token），超過則頭尾保留中間截斷

### 1-8 Memory Worker（`app/workers/memory_worker.py`）

- [ ] Redis 佇列 key：`chat:memory:queue`，每筆為 JSON `{"session_uid": "...", "message_uid": "..."}`
- [ ] DLQ key：`chat:memory:dlq`
- [ ] 主迴圈 `async def run()`：
  ```
  while True:
    ...
    for each item:
      ...
      if len(batch_buffer[session_uid]) >= batch_size OR idle > idle_seconds:
        ...
        clear buffer
  ```
- [ ] `_process_batch(session_uid, message_uids, db)`：
  1. 讀取原文 messages
...
- [ ] `chat_service.send_message` 寫完 assistant message 後：
  ```python
...
- [ ] `async def retrieve(session_uid, query_text, db) -> list[ChatMemory]`
...
- [ ] 若非空則在 Skills 區塊後附加：
  ...
  ```
- [ ] **不貼原始訊息內容**（隱私 + token 節流）

修改 `send_message`：
- [ ] 取得 user 訊息後 → 呼叫 `rag_service.retrieve(session_uid, user_content)` → 取得 `memories` → 傳入 `_build_system_prompt`

### 1-12 Session 軟刪連動

- [ ] `chat_service.delete_session` 內呼叫 `chat_memory_repository.soft_delete_by_session`（實作為 hard delete，v1.1 無需保留記憶）

### 1-13 Router 補充

- [ ] `GET /api/v1/chat/sessions/{uid}/memories`：列出 session 記憶（Admin 不可，僅擁有者；供 debug / 使用者查看用）
- [ ] 訊息內容隱私：僅回 `keywords` / `entities` / `topic` / `created_at`，不回 embedding
...
- [ ] `types/chat.ts` 新增 `ChatMemory`（不含 embedding）
- [ ] `store/chatApi.ts` 新增 `useListSessionMemoriesQuery({ sessionUid })`

### 2-2 UI

- [ ] `/sessions/[uid]/page.tsx` 右側抽屜（可收合）新增「記憶」分頁
...
- [ ] `/admin/settings` 頁已存在；確認新 seed 的 7 個 key 會自動載入顯示
...
- [ ] V22 migration 後 `chat_memory` 表存在、HNSW 索引建立成功
- [ ] V23 seed 後 `/admin/settings` 可看到 `memory.*` 與 `rag.*` 共 7 組設定
- [ ] 對話超過 5 則後（或 idle 60s）`chat_memory` 表有新記錄
- [ ] 新建 session 第一輪對話：無記憶，RAG 區塊為空、不出錯
- [ ] 第二輪對話：若 RAG 檢索到相關記憶，`system prompt` 含 `<memory>` 區塊（可 log 檢查）
- [ ] 純問候訊息（如 "hi"）**不會**產生 memory record
- [ ] 超長訊息（> 2000 token）被截斷後仍能處理
- [ ] 關閉 `rag.enabled` 後，對話正常但不注入記憶
